...
const exchange_name = 'uniswap';             // TODO: fill in the name of your exchange

const token_name = 'HUST';                // TODO: replace with name of your token
const token_symbol = 'RHUST';              // TODO: replace with symbol for your token
...
async function getPoolState() {
  ...
  let token_from_account = await token_contract.connect(provider.getSigner(defaultAccount)).balanceOf(defaultAccount);
  let liquidity_eth = await provider.getBalance(exchange_address);
  let token_amount = await exchange_contract.connect(provider.getSigner(defaultAccount)).getTokenAmount()
  return {
    ...
    token_from_account: Number(token_from_account),
...
async function getTokenFromAccount() {
  return exchange_contract.connect(provider.getSigner(defaultAccount)).getTokenFromAccount();
}

async function getTokenAmount() {
  return exchange_contract.connect(provider.getSigner(defaultAccount)).getTokenAmount();
}
...
provider.listAccounts().then((response) => {
  ...
  init().then(() => {
    ...
    getPoolState().then((poolState) => {
      ...
      $("#token-reserves").html("token-reserves " + poolState['token_liquidity'] + " " + token_symbol);
      $("#eth-reserves").html("eth-reserves " + poolState['eth_liquidity'] + " ETH");
      $("#token-from-account").html("token-from-account " + poolState['token_from_account'] + " " + token_symbol);
      $("#token-amount").html("token-amount " + poolState['token_amount'] + " " + token_symbol);
    });
  });
});
...
const sanityCheck = async function () {
  ...
  // LP provider rewards implemented
    ...
    score += check(
      ...
      swap_fee[0],
      (state6.eth_liquidity - (state5.eth_liquidity - 90)).abs() < 5 &&
      (state6.token_liquidity - (state5.token_liquidity - expected_tokens_removed)).abs() <
      expected_tokens_removed * 1.2 &&
      int.parse(user_tokens6) > int.parse(user_tokens5),
    );
  }
